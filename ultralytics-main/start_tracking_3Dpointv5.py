...
def main():
    # 初始化ROS节点
    rospy.init_node('person_3d_tracker', anonymous=True)
    
    # 创建发布者 - 发布单个人员的3D坐标
    person_3d_pub = rospy.Publisher('/person/detection/3d_position', PointStamped, queue_size=10)
    
    # 创建发布者 - 发布多个人员的3D坐标数组
    persons_3d_pub = rospy.Publisher('/person/detections/3d_positions', Point, queue_size=10)
            ...
            for result in results:
                ...
                for i, box in enumerate(boxes):
                    ...
                    if point_3d is not None:
                        ...
                        # 在控制台输出检测信息
                        print(f"检测到人员: 2D位置 ({center_x}, {center_y}), 3D位置 ({x:.2f}, {y:.2f}, {z:.2f})m, 置信度: {conf:.2f}")
            
            # 发布3D坐标
            if detected_persons:
                # 发布第一个检测到的人员的3D坐标（带时间戳）
                first_person = detected_persons[0]
                point_msg = PointStamped()
                point_msg.header.stamp = rospy.Time.now()
                point_msg.header.frame_id = "camera_link"  # 坐标系，根据实际情况修改
                point_msg.point.x = first_person['3d_position'][0]
                point_msg.point.y = first_person['3d_position'][1]
                point_msg.point.z = first_person['3d_position'][2]
                person_3d_pub.publish(point_msg)
                
                # 发布所有检测到的人员的3D坐标
                for person in detected_persons:
                    p = Point()
                    p.x = person['3d_position'][0]
                    p.y = person['3d_position'][1]
                    p.z = person['3d_position'][2]
                    persons_3d_pub.publish(p)
