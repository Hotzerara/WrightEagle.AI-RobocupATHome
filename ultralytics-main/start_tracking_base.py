...
def transform_point_to_base_link(point_camera, transformation_matrix):
    ...
    try:
        ...
        tf_buffer.can_transform("base_link", "right_gripper_link", rospy.Time.now(), rospy.Duration(1.0))
        gripper_to_base_transform = tf_buffer.lookup_transform("base_link", "right_gripper_link", rospy.Time(0))
            ...
            for result in results:
                ...
                for i, box in enumerate(boxes):
                    ...
                    if point_3d is not None:
                        ...
                        point_camera = np.array([x, y, z])  # (x, y, z)
    
                        # 示例：相机到right_gripper_link的4x4转换矩阵
                        # 注意：这是一个示例矩阵，您需要替换为实际的标定结果
                        transformation_matrix = np.array([
                            [-0.035586519487258064, -0.41531906834118093, 0.9089794668214436, -0.014120096093859223],
                            [0.017061699887913295, -0.9096755035488404, -0.41496912733370567, 0.06269794895416197],
                            [0.9992209455509413, 0.000741427930711345, 0.03945823433588758, -0.011994062531451423],
                            [0.0, 0.0, 0.0, 1.0]
                        ])
