...
class YOLO3DDetector:
    def __init__(self, args):
        ...
        # 创建显示窗口
        cv2.namedWindow('YOLO 3D Detection', cv2.WINDOW_NORMAL)
        cv2.resizeWindow('YOLO 3D Detection', 800, 600)
        ...
        try:
            ...
            for result in results:
                ...
                for i, box in enumerate(boxes):
                    ...
                    print(f"检测到对象 - 2D中心坐标: ({center_x:.1f}, {center_y:.1f}), 置信度: {conf:.2f}")
                    ...
                    if point_3d is not None:
                        ...
                        print(f"对象3D位置 - 相机坐标系: ({x:.2f}, {y:.2f}, {z:.2f})m")
                        ...
                        if point_base is not None:
                            print(f"对象3D位置 - base_link坐标系: ({point_base[0]:.2f}, {point_base[1]:.2f}, {point_base[2]:.2f})m")
            ...
            # 显示图像
            cv2.imshow('YOLO 3D Detection', display_image)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="YOLOv8 Segmentation with 3D Position Detection")
