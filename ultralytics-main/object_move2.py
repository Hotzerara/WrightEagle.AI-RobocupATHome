...
class AdvancedAppleGripperController:
    def __init__(self):
        ...
        self.safe_height = 0.0  # 20cm above table/surface
    ...
    def calculate_target_orientation(self, direction_vector, reference_direction=[1, 0, 0], twist_angle=1.57):
        """
        计算朝向目标方向的旋转四元数，可控制绕自身轴的旋转角度
        
        参数:
            direction_vector: 目标方向向量 [x, y, z]
            twist_angle: 绕自身轴旋转的角度（弧度），默认0
            reference_direction: 参考方向，默认为X轴正方向 [1, 0, 0]
            
        返回:
            list: 四元数 [x, y, z, w] (xyzw格式)
        """
        u = np.array(reference_direction, dtype=np.float64)
        v = np.array(direction_vector, dtype=np.float64)
        ...
        if cos_theta > 0.99999:
            ...
            q_base = [axis[0], axis[1], axis[2], 0.0]
        else:
            # 一般情况
            if axis_norm < 1e-10:
                q_base = [0.0, 0.0, 0.0, 1.0]
                ...
                axis = axis / axis_norm
                half_angle = math.acos(max(min(cos_theta, 1.0), -1.0)) / 2.0
        ...
        # 计算绕自身轴（目标方向）的旋转四元数
        twist_half = twist_angle / 2.0
        q_twist = [
            v[0] * math.sin(twist_half),
            v[1] * math.sin(twist_half),
            v[2] * math.sin(twist_half),
            math.cos(twist_half)
        ]
        
        # 组合旋转：先基础旋转对齐方向，再绕自身轴旋转
        return self.quaternion_multiply(q_twist, q_base)
    ...
    #     # Calculate yaw using arccos (rotation around Z axis)
    #     # yaw = cos⁻¹(√(dx²+dy²)/dx) but need to handle quadrant correctly
    #     # xy_magnitude = math.sqrt(dx**2 + dy**2)
    #     # if abs(dx) > 1e-10:  # Avoid division by zero
    #     #     yaw = math.acos(xy_magnitude / dx) if dx > 0 else math.acos(xy_magnitude / dx) + math.pi
    #     # else:
    #     #     yaw = math.pi/2 if dy > 0 else -math.pi/2
        
    #     # # Calculate pitch using arccos (rotation around Y axis)
    #     # # pitch = cos⁻¹(√(dx²+dy²)/√(dx²+dy²+dz²)) but √(dx²+dy²+dz²) = 1 since normalized
    #     # xz_magnitude = math.sqrt(dx**2 + dz**2)
    #     # if abs(dz) > 1e-10:  # Avoid division by zero
    #     #     pitch = math.acos(xz_magnitude / dz) if dx > 0 else math.acos(xz_magnitude / dz) + math.pi
    #     # else:
    #     #     pitch = math.pi/2 if dy > 0 else -math.pi/2
        
    #     # # Adjust pitch sign based on dz
    #     # zy_magnitude = math.sqrt(dz**2 + dy**2)
    #     # if abs(dy) > 1e-10:  # Avoid division by zero
    #     #     roll = math.acos(zy_magnitude / dy) if dx > 0 else math.acos(zy_magnitude / dy) + math.pi
    #     # else:
    #     #     roll = math.pi/2 if dy > 0 else -math.pi/2

    #     yaw = math.atan2(dy, dx)
    #     pitch = math.atan2(dz, math.sqrt(dx**2 + dy**2))   

    #     # No roll (keep gripper upright)
    #     roll = 1.57
    ...
    #     quaternion = quaternion_from_euler(yaw, pitch, roll, 'rzyx')
    ...
    def apple_base_position_callback(self, msg):
        ...
        if self.movement_completed:
            return  # Ignore all future callbacks after first movement
    ...
    def move_to_apple(self):
        ...
        if self.movement_completed:
            return  # Already done one movement
        ...
        try:
            rospy.loginfo("Starting ONE-TIME movement towards apple...")
    ...
    def run(self):
        """Main loop - waits for one movement to complete then spins"""
        rospy.loginfo("Controller started - will execute ONE movement then stop")
        rospy.spin()  # Just spin and let the callback handle the one-time movement
