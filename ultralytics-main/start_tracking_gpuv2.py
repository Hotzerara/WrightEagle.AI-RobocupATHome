...
def main():
    ...
    try:
        # 使用预训练的姿态估计模型，并指定设备
        model = YOLO('yolov8n-pose.pt').to(device)  # 专门用于姿态估计的模型
        # 使用半精度浮点数(f16)进一步加速GPU推理
        model.fuse()
    except Exception as e:
        print(f"加载模型失败: {e}")
        pipeline.stop()
    ...
    try:
        ...
        while True:
            ...
            # 使用YOLOv8进行姿态估计 (使用GPU加速)
            results = model(color_image, verbose=False, device=device)  # verbose=False减少控制台输出
            ...
            cv2.putText(display_image, f"FPS: {fps:.1f}", (10, 70), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            cv2.putText(display_image, f"Device: {device.upper()}", (10, 110), 
